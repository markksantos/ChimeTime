...
final class NotchAnimator: ObservableObject {
    ...
    func showNotification(for date: Date) {
        showNotchDrop(date: date, customLabel: nil)
    }

    func showNotification(withLabel label: String) {
        showNotchDrop(date: Date(), customLabel: label)
    }

    private func showNotchDrop(date: Date, customLabel: String?) {
